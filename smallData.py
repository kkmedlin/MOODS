...
#initial download
    ...
    if option==5:
        ...
        X_train, X_test, y_train, y_test, X_gender, X_gender_test = train_test_split(features, labels, onehot_encoded, random_state=10, test_size=0.4)
        temp = X_train.values
        ...
        from sklearn.model_selection import train_test_split
        x_train_np, x_test_np, y_train_np, y_test_np = train_test_split(X,encoded_Y, test_size=0.4, random_state=rs) 
        x_test_np, x_valid_np, y_test_np, y_valid_np = train_test_split(x_test_np,y_test_np, test_size=0.5, random_state=rs2) 
        print("TESTING VALID np x and y validate:", x_valid_np.shape, y_valid_np.shape)
    ...
    if option!=6:
        ...
        print("Validation data, counts of label '1': {}".format(sum(y_valid_np==1)))
        print("Validation data, counts of label '0': {} \n".format(sum(y_valid_np==0)))
        print("np x and y train:", x_train_np.shape, y_train_np.shape)
        print("np x and y test:", x_test_np.shape, y_test_np.shape)
        print("np x and y validate:", x_valid_np.shape, y_valid_np.shape)
