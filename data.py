...
#initial download
    ...
    X_train, X_test, y_train, y_test, X_gender, X_gender_test = train_test_split(features, labels, onehot_encoded, test_size=0.40, random_state=10)
    X_test, X_valid, y_test, y_valid, X_gender_test, X_gender_valid = train_test_split(features, labels, onehot_encoded, test_size=0.50, random_state=10)
    ...
    temp3 = X_valid.values
    x_valid_np = np.concatenate((temp3, X_gender_valid), axis=1)
    ...
    print("Valid data, counts of label '1': {}".format(sum(y_valid_np==1)))
    print("Valid data, counts of label '0': {} \n".format(sum(y_valid_np==0)))
    print("np x and y train:", x_train_np.shape, y_train_np.shape)
    print("np x and y test:", x_test_np.shape, y_test_np.shape)
    print("np x and y valid:", x_valid_np.shape, y_valid_np.shape)
...
#initial download
    ...
    from sklearn.model_selection import train_test_split
    x_train_np, x_test_np, y_train_np, y_test_np=train_test_split(X,Y,random_state=0,test_size=0.4)
    x_test_np, x_valid_np, y_test_np, y_valid_np=train_test_split(x_test_np,y_test_np,random_state=0,test_size=0.5)
...
def shuttle():
    ...
    if option==8:
        ...
        x_test_np, x_valid_np, y_test_np, y_valid_np = train_test_split(x_test_np, y_test_np, test_size=0.50, random_state=1)
        ep=10
        ne=1000
    if option!=6:
        ...
        print("Before OverSampling, VALIDATION counts of label '1': {}".format(sum(y_valid_np==1)))
        print("Before OverSampling, VALIDATION counts of label '0': {} \n".format(sum(y_valid_np==0)))
    return x_train_np, y_train_np, x_test_np, y_test_np, x_valid_np, y_valid_np
...
def connect4():
    ...
    x_train_np, x_test_np, y_train_np, y_test_np = train_test_split(X, y, test_size=0.40, random_state=1)
    x_test_np, x_valid_np, y_test_np, y_valid_np = train_test_split(x_test_np, y_test_np, test_size=0.50, random_state=1)
    ...
    print("Connect4 Valid data, counts of label '1': {}".format(sum(y_valid_np==1)))
    print("Connect4 Valid data, counts of label '0': {} \n".format(sum(y_valid_np==0)))
    print("np x and y train:", x_train_np.shape, y_train_np.shape)
    print("np x and y test:", x_test_np.shape, y_test_np.shape)
    print("np x and y test:", x_valid_np.shape, y_valid_np.shape)
...
#def create_imbalanced_samplers(x_train, y_train, x_test, y_test, x_valid, y_valid, y_train_np, y_test_np, y_valid_np):
    ...
    valid_data_tensor = torch.cat((x_valid,y_valid[...,None]),-1)
    minority_valid_data_tensor = valid_data_tensor[minority_valid_indices]
    majority_valid_data_tensor = valid_data_tensor[majority_valid_indices]
    ...
    majority_sample = majority_train_data_tensor[torch.randint(0, len(majority_train_data_tensor), (majority_size_train,))]
    ...
    minority_size_valid = minority_valid_data_tensor.shape[0]
    majority_size_valid = majority_valid_data_tensor.shape[0]
    majority_sample_valid = majority_valid_data_tensor[torch.randint(0, len(majority_valid_data_tensor), (majority_size_valid,))]
    minority_sample_valid = minority_valid_data_tensor[torch.randint(0, len(minority_valid_data_tensor), (minority_size_valid,))]
    current_sample_valid = torch.cat((minority_sample_valid,majority_sample_valid),0)
    y_label_valid = current_sample_valid[...,:,-1]
    y_label_valid = y_label_valid.long()
    X_state_valid = current_sample_valid[...,:,0:-1]
    
    return X_state, y_label, X_state_test, y_label_test, X_state_valid, y_label_valid, majority_train_data_tensor, minority_train_data_tensor 
